...
dynamic_trace = BASEDIR + "traces/" + QUERY + "/rowStore/x86/output_trace.out.tid0.dyn.out"
memory_trace = BASEDIR + "traces/" + QUERY + "/rowStore/x86/output_trace.out.tid0.mem.out"
static_trace = BASEDIR + "traces/" + QUERY + "/rowStore/x86/output_trace.out.tid0.stat.out"
################### TREATING FILE INPUT ###################
            ...
            if column == numberOfPredicates - 1:
                for i in range(sizeOfMaterialization):
                    ...
                    memory_block[column][tuple] += str(
                        "R 16 " + str(
                            address_base[column] - (REGISTER_SIZE)) + " " + str(
                            basicBlock + 3) + "\n")
                    address_base[column] += (REGISTER_SIZE)
                    memory_block[column][tuple] += str(
                        "W 16 " + str(address_target[column]) + " " + str(
                            basicBlock + 3) + "\n")
                    address_target[column] += (REGISTER_SIZE)
        elif column > 0:
            if column == numberOfPredicates - 1:
                fieldCount = DATA_SIZE + 1
            if lastSum > 0:
...
os.system("rm -f " + BASEDIR + "traces/" + QUERY + "/rowStore/x86/" + "*gz")
os.system("gzip " + BASEDIR + "traces/" + QUERY + "/rowStore/x86/" + "*.out")
print "ALL Done!"
