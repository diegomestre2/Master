...
for HMC_OPERATION_CAPACITY in (16, 32, 64, 128, 256):
    ...
    lastFieldsSum = 0
    loadSize = 16  # Bytes
    vectorSizeMaxToWrite = HMC_OPERATION_CAPACITY
            ...
            if fieldsByInstruction == 1:
                ...
                if bitColSum[column] > 0 or column == 0:
                    ...
                    if column > 0:
                        ...
                        memory_block[column][tuple] += (
                            "R " + str((HMC_OPERATION_CAPACITY / loadSize) * 2) + " " + str(
                                address_target_bitmap[column - 1] - 1) + " " + str(
                                basicBlock + 3) + "\n")
                        address_target_bitmap[column - 1] += ((HMC_OPERATION_CAPACITY / loadSize) * 2)
                    # if column > 0
                    for i in range(HMC_OPERATION_CAPACITY / vectorSizeMaxToWrite):
                        dynamic_block[column][tuple] += str(str(basicBlock + 4) + "\n")
                        memory_block[column][tuple] += str(
                            "W " + str((vectorSizeMaxToWrite / loadSize) * 2) + " " + str(
                                address_target_bitmap[column]) + " " + str(basicBlock + 4) + "\n")
                        address_target_bitmap[column] += ((vectorSizeMaxToWrite / loadSize) * 2)
                    # for
                elif column > 0:
                    if column == numberOfPredicates - 1:
                        fieldsByInstruction = HMC_OPERATION_CAPACITY + 1
    ...
    os.system("rm -f " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/"  + str(HMC_OPERATION_CAPACITY) + "/unrolled4x/" + "*gz")
    os.system("gzip " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/"  + str(HMC_OPERATION_CAPACITY) + "/unrolled4x/" + "*.out")
    print "ALL Done!"
            ...
            if fieldsByInstruction == 1:
                ...
                if bitColSum[column] > 0 or column == 0:
                    ...
                    for i in range(8):
                        ...
                        address_base[column] += HMC_OPERATION_CAPACITY
                    # for
                    if HMC_OPERATION_CAPACITY >= 128:
                        vectorSizeMaxToWrite = 64
                    ########################################################################
                    # CREATE THE BITMAP 1 Byte of Store by 32 Bytes of Loads
                    ########################################################################
                    if column > 0:
                        dynamic_block[column][tuple] += str(str(basicBlock + 3) + "\n")
                        memory_block[column][tuple] += (
                            "R " + str((HMC_OPERATION_CAPACITY / loadSize) * 4) + " " + str(
                                address_target_bitmap[column - 1] - 1) + " " + str(
                                basicBlock + 3) + "\n")
                        address_target_bitmap[column - 1] += ((HMC_OPERATION_CAPACITY / loadSize) * 4)
                    # if column > 0
                    for i in range(HMC_OPERATION_CAPACITY / vectorSizeMaxToWrite):
                        dynamic_block[column][tuple] += str(str(basicBlock + 4) + "\n")
                        memory_block[column][tuple] += str(
                            "W " + str((vectorSizeMaxToWrite / loadSize) * 4) + " " + str(
                                address_target_bitmap[column]) + " " + str(basicBlock + 4) + "\n")
                        address_target_bitmap[column] += ((vectorSizeMaxToWrite / loadSize) * 4)
                    # for
                elif column > 0:
                    if column == numberOfPredicates - 1:
                        fieldsByInstruction = HMC_OPERATION_CAPACITY + 1
    ...
    print "Compressing Files..."
    os.system("rm -f " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(HMC_OPERATION_CAPACITY) + "/unrolled8x/" + "*gz")
    os.system("gzip " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(HMC_OPERATION_CAPACITY) + "/unrolled8x/" + "*.out")
    print "ALL Done!"
   ...
   ########### UNROLLED 16x ########################
    ...
    input_file = BASEDIR + "bitmap_files/resultQ06.txt"
    dynamic_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(
        HMC_OPERATION_CAPACITY) + "/unrolled16x/output_trace.out.tid0.dyn.out"

    memory_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(
        HMC_OPERATION_CAPACITY) + "/unrolled16x/output_trace.out.tid0.mem.out"

    static_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(
        HMC_OPERATION_CAPACITY) + "/unrolled16x/output_trace.out.tid0.stat.out"
    ...
    basicBlock = 0
    print("Generating Traces Files For HMC unrolled 16x... " + str(HMC_OPERATION_CAPACITY) + " Bytes")
    #################### STATIC FILE #########################
    print "Generating Static File..."
    for tuple in range(numberOfPredicates):
        basicBlock += 1
            ...
            if fieldsByInstruction == 1:
                ...
                if bitColSum[column] > 0 or column == 0:
                    ...
                    for i in range(16):
                        ...
                        address_base[column] += HMC_OPERATION_CAPACITY
                    # for
                    if HMC_OPERATION_CAPACITY >= 64:
                        vectorSizeMaxToWrite = 32
                    ########################################################################
                    # CREATE THE BITMAP 1 Byte of Store by 32 Bytes of Loads
                    ########################################################################
                    if column > 0:
                        dynamic_block[column][tuple] += str(str(basicBlock + 3) + "\n")
                        memory_block[column][tuple] += (
                            "R " + str((HMC_OPERATION_CAPACITY / loadSize) * 8) + " " + str(
                                address_target_bitmap[column - 1] - 1) + " " + str(
                                basicBlock + 3) + "\n")
                        address_target_bitmap[column - 1] += ((HMC_OPERATION_CAPACITY / loadSize) * 8)
                    # if column > 0
                    for i in range(HMC_OPERATION_CAPACITY / vectorSizeMaxToWrite):
                        dynamic_block[column][tuple] += str(str(basicBlock + 4) + "\n")
                        memory_block[column][tuple] += str(
                            "W " + str((vectorSizeMaxToWrite / loadSize) * 8) + " " + str(
                                address_target_bitmap[column]) + " " + str(basicBlock + 4) + "\n")
                        address_target_bitmap[column] += ((vectorSizeMaxToWrite / loadSize) * 8)
                    # for
                elif column > 0:
                    if column == numberOfPredicates - 1:
                        fieldsByInstruction = HMC_OPERATION_CAPACITY + 1
    ...
    print "Compressing Files..."
    os.system("rm -f " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(HMC_OPERATION_CAPACITY) + "/unrolled16x/" + "*gz")
    os.system("gzip " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(HMC_OPERATION_CAPACITY) + "/unrolled16x/" + "*.out")
    print "ALL Done!"
   ...
   ########### UNROLLED 32x ########################
    ...
    input_file = BASEDIR + "bitmap_files/resultQ06.txt"
    dynamic_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(
        HMC_OPERATION_CAPACITY) + "/unrolled32x/output_trace.out.tid0.dyn.out"

    memory_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(
        HMC_OPERATION_CAPACITY) + "/unrolled32x/output_trace.out.tid0.mem.out"

    static_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(
        HMC_OPERATION_CAPACITY) + "/unrolled32x/output_trace.out.tid0.stat.out"
    ...
    basicBlock = 0
    print("Generating Traces Files For HMC unrolled 32x... " + str(HMC_OPERATION_CAPACITY) + " Bytes")
    #################### STATIC FILE #########################
    print "Generating Static File..."
    for tuple in range(numberOfPredicates):
        basicBlock += 1
            ...
            if fieldsByInstruction == 1:
                ...
                if bitColSum[column] > 0 or column == 0:
                    ...
                    for i in range(32):
                        ...
                        address_base[column] += HMC_OPERATION_CAPACITY
                    # for
                    if HMC_OPERATION_CAPACITY >= 32:
                        vectorSizeMaxToWrite = 16
                    ########################################################################
                    # CREATE THE BITMAP 1 Byte of Store by 32 Bytes of Loads
                    ########################################################################
                    if column > 0:
                        dynamic_block[column][tuple] += str(str(basicBlock + 3) + "\n")
                        memory_block[column][tuple] += (
                            "R " + str((HMC_OPERATION_CAPACITY / loadSize) * 16) + " " + str(
                                address_target_bitmap[column - 1] - 1) + " " + str(
                                basicBlock + 3) + "\n")
                        address_target_bitmap[column - 1] += ((HMC_OPERATION_CAPACITY / loadSize) * 16)
                    # if column > 0
                    for i in range(HMC_OPERATION_CAPACITY / vectorSizeMaxToWrite):
                        dynamic_block[column][tuple] += str(str(basicBlock + 4) + "\n")
                        memory_block[column][tuple] += str(
                            "W " + str((vectorSizeMaxToWrite / loadSize) * 16) + " " + str(
                                address_target_bitmap[column]) + " " + str(basicBlock + 4) + "\n")
                        address_target_bitmap[column] += ((vectorSizeMaxToWrite / loadSize) * 16)
                    # for
                elif column > 0:
                    if column == numberOfPredicates - 1:
                        fieldsByInstruction = HMC_OPERATION_CAPACITY + 1
    ...
    print "Compressing Files..."
    os.system("rm -f " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(HMC_OPERATION_CAPACITY) + "/unrolled32x/" + "*gz")
    os.system("gzip " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC/" + str(HMC_OPERATION_CAPACITY) + "/unrolled32x/" + "*.out")
    print "ALL Done!"
