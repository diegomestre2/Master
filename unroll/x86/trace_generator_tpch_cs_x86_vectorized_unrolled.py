...
dynamic_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled4x/" + "output_trace.out.tid0.dyn.out"
memory_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled4x/" + "output_trace.out.tid0.mem.out"
static_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled4x/" + "output_trace.out.tid0.stat.out"
################### TREATING FILE INPUT ###################
...
basicBlock = 0
print("Generating Traces Files For x86...Unrolled 4x - " + str(REGISTER_SIZE) + " Bytes")
#################### STATIC FILE #########################
...
for tuple in range(numberOfPredicates):
    ...
    INSTRUCTION_ADDR += 4
    FILE_STAT.write("JNE 7 " + str(INSTRUCTION_ADDR) + " 2 4 2 4 10 12 1 7 0 0 0 0 0 4 0 0 0\n")
    INSTRUCTION_ADDR += 2
    basicBlock += 1
    FILE_STAT.write("@" + str(basicBlock) + "\n")  # MATCH POSITION (STORE)#
    FILE_STAT.write("MOV 9 " + str(INSTRUCTION_ADDR) + " 6 1 7 1 13 0 0 0 0 1 3 0 0 0\n")  # W 1 Byte
    INSTRUCTION_ADDR += 6
...
input_file = BASEDIR + "bitmap_files/resultQ06.txt"
dynamic_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled8x/" + "output_trace.out.tid0.dyn.out"
memory_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled8x/" + "output_trace.out.tid0.mem.out"
static_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled8x/" + "output_trace.out.tid0.stat.out"
################### TREATING FILE INPUT ###################
FILE_INPUT = open(input_file, 'r')
...
basicBlock = 0
print("Generating Traces Files For x86...Unrolled 8x - " + str(REGISTER_SIZE) + " Bytes")
#################### STATIC FILE #########################
print "Generating Static File..."
for tuple in range(numberOfPredicates):
    basicBlock += 1
    ...
    basicBlock += 1
    FILE_STAT.write("@" + str(basicBlock) + "\n")  # MATCH POSITION (STORE)#
    FILE_STAT.write("MOV 9 " + str(INSTRUCTION_ADDR) + " 6 1 7 1 10 0 0 0 0 1 3 0 0 0\n")  # W 1 Byte
    INSTRUCTION_ADDR += 6
...
os.system("rm -f " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled8x/" + "*gz")
os.system("gzip " + BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/x86/" + str(
    REGISTER_SIZE) + "/unrolled8x/" + "*.out")
print "ALL Done!"
