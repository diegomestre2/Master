...
for SELECTIVITY in ("0001", "001", "01", "1", "10", "100"):
    ...
    dynamic_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC_NEW/" + str(
        HMC_OPERATION) + "/predication/selectivity/" + SELECTIVITY + "/output_trace.out.tid0.dyn.out"
    memory_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC_NEW/" + str(
        HMC_OPERATION) + "/predication/selectivity/" + SELECTIVITY + "/output_trace.out.tid0.mem.out"
    static_trace = BASEDIR + "traces/" + QUERY + "/columnStore/" + QUERY_ENGINE + "/HMC_NEW/" + str(
        HMC_OPERATION) + "/predication/selectivity/" + SELECTIVITY + "/output_trace.out.tid0.stat.out"
    ...
    basicBlock = 0
    print("Generating Traces Files For HMC_NEW... Predication Unrolled 32x " + str(
        HMC_OPERATION) + " Bytes Selectivity " + SELECTIVITY)
    #################### STATIC FILE #########################
    ...
    for i in range(numberOfPredicates):
        ...
        basicBlock += 1
        FILE_STAT.write("@" + str(basicBlock) + "\n")  # APPLY READ BITMAP)#
        FILE_STAT.write("HMC_LD 16 " + str(INSTRUCTION_ADDR) + " 4 2 7 11 0 0 0 1 0 0 3 0 0 1 -1 -1 33\n")  # R
        INSTRUCTION_ADDR += 4
