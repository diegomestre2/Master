...
for SELECTIVITY in ("0001", "001", "01", "1", "10", "100"):
    ...
    basicBlock = 0
    print("Generating Traces Files For HMC...Unrolled 32x - " + str(HMC_OPERATION) + " Bytes Selectivity" + SELECTIVITY)
    #################### STATIC FILE #########################
    ...
    for tuple in range(numberOfPredicates):
        ...
        basicBlock += 1
        FILE_STAT.write("@" + str(basicBlock) + "\n")  # READ BITMAP)#
        FILE_STAT.write("CMP 1 " + str(INSTRUCTION_ADDR) + " 4 2 13 7 1 9 0 0 1 0 0 3 0 0 0\n")  # R 1 Bytes
        INSTRUCTION_ADDR += 4
        FILE_STAT.write("CMP 1 " + str(INSTRUCTION_ADDR) + " 4 1 14 1 2 0 0 1 0 0 3 0 0 0\n")  # R 1 Bytes
        INSTRUCTION_ADDR += 4
        FILE_STAT.write("CMP 1 " + str(INSTRUCTION_ADDR) + " 4 1 15 1 3 0 0 1 0 0 3 0 0 0\n")  # R 1 Bytes
        INSTRUCTION_ADDR += 4
        FILE_STAT.write("CMP 1 " + str(INSTRUCTION_ADDR) + " 4 1 16 1 4 0 0 1 0 0 3 0 0 0\n")  # R 1 Bytes
        INSTRUCTION_ADDR += 4
        ...
        INSTRUCTION_ADDR += 6
        FILE_STAT.write("MOV 9 " + str(INSTRUCTION_ADDR) + " 6 1 8 1 15 0 0 0 0 1 3 0 0 0\n")  # W
        INSTRUCTION_ADDR += 6
        FILE_STAT.write("MOV 9 " + str(INSTRUCTION_ADDR) + " 6 1 8 1 16 0 0 0 0 1 3 0 0 0\n")  # W
        INSTRUCTION_ADDR += 6
                ...
                if bitColSum[column] > 0 or column == 0:
                    ...
                    for i in range(4):
                        memory_block[column][tuple] += str(
                            "W " + str(bitmapSize / 4) + " " + str(address_target_bitmap[column]) + " " + str(
                                basicBlock + 4) + "\n")
                        address_target_bitmap[column] += bitmapSize / 4
                elif column > 0:
                    if column == numberOfPredicates - 1:
                        fieldsByInstruction = (HMC_OPERATION * 8) + 1
                    ########################################################################
                    ...
                    for i in range(4):
                        memory_block[column][tuple] += (
                            "R " + str(bitmapSize / 4) + " " + str(address_target_bitmap[column - 1] - 1) + " " + str(
                                basicBlock + 2) + "\n")
                        bitmapAddr += 32
